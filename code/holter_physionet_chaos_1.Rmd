...
create_plot <- function(metric_name, custom_title = NULL) {
  ...
  p <- ggplot(plot_data, aes(x = condition, y = value, fill = condition)) +
    ...
    scale_fill_manual(values = c("Healthy" = "#7FCDBB", "CHF" = "#F8AFA8", "AF" = "#BFA9DF")) +
...
# Create a title grob
title_grob <- textGrob("Downsampled 24-Hour Holter to 5 Minute Heart Rate", gp=gpar(fontsize=16, fontface="bold"))
footnote_grob <- textGrob("Dataset: Physionet Heart Chaos Competition (Healthy (n=5); CHF (n=5); AF (n=5))", gp=gpar(fontsize=14, fontface="italic"), just="left", x=unit(0.01, "npc"))
