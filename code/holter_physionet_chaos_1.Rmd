...
combined_df <- combined_df %>%
  ...
  mutate(
    status = case_when(
      condition %in% c("Normal") ~ "Healthy",
      condition %in% c("AF", "CHF") ~ "Unhealthy",
...
#df_metrics <- data_holter_downsampled_metrics
...
ggplot(loadings_df, aes(x = PC, y = variable, fill = loading)) +
  geom_tile() +
  scale_fill_gradient2(low = "cornflowerblue", mid = "white", high = "mediumvioletred", midpoint = 0) +
...
ggplot(scores, aes(x = PC1, y = PC2,
  ...
  geom_point(size = 3) +
  labs(
    title = "PCA of Heart Rate Time Series Metrics",
    #title = "PC1 vs PC2",
    x = pc1_lab,
    y = pc2_lab,
    color = "Condition"
  ) +
  scale_color_manual(values = c("Normal" = "darkorange1", "CHF" = "green4", "AF" = "royalblue3")) +
#  scale_fill_manual(values = c("Normal" = "orange", "CHF" = "palegreen3", "AF" = "skyblue2")) +
#+scale_color_manual(values = c("Normal" = "chartreuse3", "CHF" = "mediumvioletred", "AF" = "cornflowerblue")) +
...
create_plot <- function(metric_name, custom_title = NULL) {
  ...
  p <- ggplot(plot_data, aes(x = condition, y = value, fill = condition)) +
    ...
    scale_fill_manual(values = c("Normal" = "darkolivegreen2", "CHF" = "plum2", "AF" = "skyblue2")) +
...
# Create a simplified plot for each metric
create_plot_simple <- function(metric_name, custom_title = NULL) {
  # Filter data for the specific metric
  plot_data <- df_long %>% filter(metric == metric_name)
  
  # Get the unique condition groups
  condition_groups <- unique(plot_data$condition)
  ...
  # Create the base plot with condition
  p <- ggplot(plot_data, aes(x = condition, y = value, fill = condition)) +
    geom_boxplot(alpha = 0.7, outlier.size = 2) +
    # Add mean points with labels
    geom_point(data = means, aes(y = mean_value), shape = 18, size = 4, color = "black") +
 ...
 #   scale_fill_manual(values = c("Normal" = "darkolivegreen2", "CHF" = "plum2", "AF" = "skyblue2")) +
  #  scale_fill_manual(values = c("Normal" = "orange", "CHF" = "palegreen3", "AF" = "skyblue2")) +
    scale_fill_manual(values = c("Normal" = "darkorange1", "CHF" = "green4", "AF" = "royalblue3")) +
    labs(
      title = plot_title,
      x = NULL,  # Remove x-axis label as it's redundant
      y = "Value"
...
# Custom titles mapping - you can modify these titles
metric_titles <- list(
  "avg_hr_DN_HistogramMode_5" = "Histogram Mode (5 Bins)",
  "avg_hr_cv" = "Coefficient of Variation",
  "avg_hr_SB_BinaryStats_diff_longstretch0" = "Longest Consecutive Decrease",
  "avg_hr_PD_PeriodicityWang_th0_01" = "Wang Periodicity Index",
  "avg_hr_IN_AutoMutualInfoStats_40_gaussian_fmmi" = "Auto Mutual Information",
  "avg_hr_CO_FirstMin_ac" = "First Autocorrelation Minimum",
  "avg_hr_permen" = "Permutation Entropy"
)

# Use the specified metrics list
metrics_to_plot <- c(#"avg_hr_DN_HistogramMode_5", 
...
for (i in seq_along(metrics_to_plot)) {
  ...
  plot_list[[i]] <- create_plot_simple(metric, custom_title)
}
...
# ggsave("heart_rate_metrics.png", grid_plot, width = 12, height = 10)
...
#  status        n   mean    sd
#  <chr>     <int>  <dbl> <dbl>
#1 Healthy       5  1.92  0.793
#2 Unhealthy    10 -0.958 1.08 

library(pwr)                       # base R package
pwr::pwr.t2n.test(n1 = 202,             # healthy
             n2 = 51,              # ESRD (smallest group)
          #   d  = 1.198,             # effect size
             power = 0.95,
             alternative = "two.sided")

install.packages("effsize")
effsize::cohen.d(scores %>% filter(status == "Healthy") %>% pull(PC1),
                 scores %>% filter(status == "Unhealthy") %>% pull(PC1),
...
data_holter_downsampled %>% 
  filter(id == "n3nn") %>% 
  mutate(hour = minute_index/60) %>% 
  ggplot(aes(x = hour, y = mean_hr)) + 
  labs(
    x = "Hour",
    y = "Mean Heart Rate",
    title = "Heart Rate Every 5 Minutes"
  ) +
  geom_point() +
  geom_line(alpha = 0.7) +
  theme_minimal()
