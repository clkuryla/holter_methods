...
Author: [Christine — adapt as needed]
"""
...
DATABASES = [
    {
        'name': 't002',
        'ann_dir': Path("/Volumes/YourDrive/THEW/t002/"),
        'fs': 200,
    ...
    {
        'name': 't003',
        'ann_dir': Path("/Volumes/YourDrive/THEW/t003/"),
        'fs': 200,
        'downsample_to_fs': None,
        'condition': 'Healthy',
    },
    {
        'name': 't016',
        'ann_dir': Path("/Volumes/YourDrive/THEW/t016/"),
        'fs': 1000,
...
OUTPUT_ROOT = Path("./hr_timeseries_1min/")
...
def read_thew_ann(filepath, fs, downsample_to_fs=None):
    ...
    with open(filepath, 'rb') as f:
        ...
        magic = f.read(8)
        if not magic[:3].upper().startswith(b'ANN'):
            return None
